...
def virtual_adversarial_training_fix(x, t, forward_func, fix_d,
                                     main_obj_type,
                                     epsilon,
                                     lamb=numpy.asarray(1.0, theano.config.floatX),
                                     norm_constraint='L2',
    ...
    #     Hd = theano.gradient.disconnected_grad(Hd)
    #     d = Hd
    r_vadv = get_perturbation(fix_d, epsilon, norm_constraint)
    if unchain_y:
        y_hat = theano.gradient.disconnected_grad(y)
        pred = forward_func(x + r_vadv)
        vadv_cost = get_kl(pred, y_hat, main_obj_type).mean()
    ...
    return ret  # , sup_loss, unsup_loss, T.sum(fix_d)
